...
class MvtecLOCODataset(Dataset):
    """
    MVTec LOCO 数据集: 包含 structural 和 logical anomalies.
    train only on good images; test on good + all defect types.
    """
    ...
    def __init__(
        ...
        cats = categories if categories else MVTEC_LOCO_CATEGORIES
        self.samples: list[dict] = []
        for cat in cats:
            self._collect_category(cat)
        
        if len(self.samples) == 0:
            raise RuntimeError(f"No MVTec LOCO samples found under {root}. Check `root` in config.")
    ...
    def _collect_val(self, category: str, base: Path) -> None:
        # LOCO 的 val 从 test good 中划分
        import random
        
        test_good_dir = base / "test" / "good"
        if not test_good_dir.is_dir():
            return
        
        all_good = sorted(test_good_dir.glob("*.png"))
        if not all_good:
            return
    ...
    def _collect_test(self, category: str, base: Path) -> None:
        ...
        if good_dir.is_dir():
            ...
            n = len(items)
            n_val = int(n * self.val_ratio)
            test_good = items[n_val:]  # 排除 val 部分
        ...
        for sub in sorted(test_root.iterdir()):
            ...
            for p in sorted(sub.glob("*.png")):
                ...
                if not mask_path.is_file():
                    mask_path = gt_root / defect_name / p.name
